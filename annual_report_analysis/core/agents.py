...
from ..agents.memory import LongTermMemory, ShortTermMemory
from ..agents.prompts import SECTION_GUIDANCE, AGNO_SYSTEM_PROMPTS
...
from ..agents.task_decomposition import TaskDecomposer, DecomposedTask, TaskType
...
class SupervisorAgent:
    ...
    def _process_task(
        self, task: DecomposedTask, chunk: DocumentChunk, state: WorkflowState
    ) -> None:
        """
...
class BaseSectionAgent:
    def __init__(
        self, name: SectionName, ltm: LongTermMemory, collaborative_memory=None
    ) -> None:
        self.name = name
        self.ltm = ltm
        self.stm = ShortTermMemory(capacity=20)
        ...
        if self.collaborative_memory:
            ...
            for related_section in self.related_sections.get(self.name.value, []):
                self.collaborative_memory.subscribe_to_agent(
                    self.name.value, related_section
                )
                # Add cross-references
                self.collaborative_memory.add_cross_reference(
                    self.name.value, related_section
                )

    def _analyze_with_collaboration(
        self, text: str, collaborative_insights: Dict[str, List[Any]]
    ) -> Dict[str, List[str]]:
    ...
    def handle(self, chunk: DocumentChunk, state: WorkflowState) -> None:
        ...
        prior_records = self.ltm.query_all(
            agent_name=self.name.value, section=self.name
        )
        prior_good: list[str] = []
        prior_bad: list[str] = []
        ...
        if prior_good or prior_bad:
            context_header = (
                f"Previous decisions context ({task_type}):\n- GOOD: "
                + "; ".join(prior_good[:5])
                + "\n- BAD: "
                + "; ".join(prior_bad[:5])
                + "\n\n"
        ...
        if shenanigans_patterns:
            high_risk_patterns = [
                pattern
                for pattern, score in shenanigans_patterns.items()
            ...
            if high_risk_patterns:
                if isinstance(risks, list):
                    risks.extend(
                        [
                            f"Potential financial shenanigan detected: {pattern}"
        ...
        self.stm.add(
            AgentMessage(
                sender=self.name.value, recipient=self.name.value, content=summary
            )
        )
